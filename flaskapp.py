...
def connect_db():
    sql = sqlite3.connect('/home/keybi/flaskapp/gminy.db')
...
class Kalkulator:
    ...
    def intensity2(self):
        if (self.region == 'lubuskie' or self.region == 'zachodniopomorskie') and self.size == 'duży':
            ...
            intensity = 0.45
        elif (self.region == 'lubuskie' or self.region == 'zachodniopomorskie') and (
                self.size == 'mikro' or self.size == 'mały'):
            intensity = 0.55
...
class Where:
    ...
    def wielkopolskie(self):
        ...
        db = get_db()
        cur = db.execute(
            'select * from gminy where {} <= {} and wojewodztwo = "{}"'.format(naklady, self.value, 'wielkopolskie'))
        results = cur.fetchall()
        wielkopolskie = {}
        for i in range(len(results)):
    ...
    def zachodniopomorskie(self):
        ...
        if self.currency == 'EURO':
            self.value *= cena
        db = get_db()
        cur = db.execute('select * from gminy where {} <= {} and wojewodztwo = "{}"'.format(naklady, self.value,
                                                                                            'zachodniopomorskie'))
        results = cur.fetchall()
        zachodniopomorskie = {}
        for i in range(len(results)):
...
def naklady():
    ...
    if request.method == 'GET':
        ...
        return render_template('indexnak.html', lubuskie_powiaty=lubuskie_powiaty,
